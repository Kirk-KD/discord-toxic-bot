...
class Task:
    def __init__(self, func, interval: int):
        self.func = func
        self.interval = interval
        self.timer = datetime.datetime.now()

    async def call(self, collection):
        if datetime.datetime.now() >= self.timer + datetime.timedelta(0, self.interval):
            self.timer = datetime.datetime.now() + datetime.timedelta(0, self.interval)
            await self.func(collection)

    @staticmethod
    def task(interval: int):
        def decorator(function):
            def wrapper(*args, **kwargs):
                return Task(function, interval)
    ...
    async def start_tasks(self):
        ...
        while not self.client.is_closed():
            for task in self.tasks:
                await task.call(self)

    # tasks
    @Task.task(interval=60 * 60)
